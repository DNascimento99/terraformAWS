...
```yml
    provider:
        ...
        iam:
            role:
            managedPolicies:
                - arn:aws:iam::347241299761:policy/Allow_Lambda
...
```json
{
	...
	"Statement": [
		...
		},
		{
			"Effect": "Allow",
			"Action": "SNS:Publish",
			"Resource": "arn:aws:sns:us-east-1:347241299761:bp-devops-lambda-acm-list-dev-MySNSTopic-*"
		},
		{
			"Sid": "Statement1",
			"Effect": "Allow",
...
```bash
$ aws sns subscribe --topic-arn arn:aws:sns:us-east-1:347241299761:bp-devops-lambda-acm-list-dev-MySNSTopic --protocol email --notification-endpoint 3c824140.brasilprev.com.br@br.teams.ms
```
...
Criar role com o nome **bp-devops-lambda-acm-list-dev-certificateChecker-assumeRole** em cada uma das conta da organização para que o lambda consiga assumi-las e assim listar seus respectivos certificados; a role deve conter a seguinte regra:
        ...
        DashboardBody: !Sub |
          {
            "widgets": [
              ...
              },
              {
                ...
                "height": 8,
                "properties": {
                    "endpoint": "arn:aws:lambda:us-east-1:347241299761:function:bp-devops-lambda-acm-list-dev-certificateChecker",
                    "updateOn": {
                        "refresh": true,
                        "resize": true,
                        "timeRange": true
